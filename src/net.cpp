...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Datasavercoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    ...
    {"explorer.datasaver.org", "206.189.144.34"}
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Datasavercoin is probably already running."), addrBind.ToString().c_str());
        else
