...
#if defined(NDEBUG)
# error "Datasavercoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Datasavercoin Signed Message:\n";
...
{
    // Datasavercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
   ...
   if(nHeight ==2){
    ...
    nSubsidy >>= (nHeight / 840000); // Datasavercoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 60 ; // Datasavercoin:3 minute retarget time
static const int64 nTargetSpacing = 1 * 60; // Datasavercoin: 1  minute block time
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Datasavercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Datasavercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Datasavercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DatasavercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DatasavercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DatasavercoinMiner(CWallet *pwallet)
{
    printf("DatasavercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("datasavercoin-miner");
    ...
    try { loop {
        ...
        printf("Running DatasavercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DatasavercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DatasavercoinMiner, pwallet));
}
